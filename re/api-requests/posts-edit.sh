# Title
curl -w "%{time_total}\n" -X POST -H "Content-Type: application/json" -d '{"method": "posts.save", "params": {"id": 1, "name": "Title"}, "token": "test"}' http://localhost/api/ https://web.kosyachniy.com/api/

# Content
curl -w "%{time_total}\n" -X POST -H "Content-Type: application/json" -d '{"method": "posts.save", "params": {"id": 1, "cont": "Content"}, "token": "test"}' http://localhost/api/ https://web.kosyachniy.com/api/

# Category
curl -w "%{time_total}\n" -X POST -H "Content-Type: application/json" -d '{"method": "posts.save", "params": {"id": 1, "category": 1}, "token": "test"}' http://localhost/api/ https://web.kosyachniy.com/api/

# Tags
curl -w "%{time_total}\n" -X POST -H "Content-Type: application/json" -d '{"method": "posts.save", "params": {"id": 1, "tags": ["FastAPI", "React"]}, "token": "test"}' http://localhost/api/ https://web.kosyachniy.com/api/

# Cover
curl -w "%{time_total}\n" -X POST -H "Content-Type: application/json" -d '{"method": "posts.save", "params": {"id": 1, "cover": "data:image/jpeg;base64,"}, "token": "test"}' http://localhost/api/ https://web.kosyachniy.com/api/
